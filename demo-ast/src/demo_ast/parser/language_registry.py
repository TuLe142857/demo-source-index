...
class LanguageRegistry:
    def __init__(self, configs: Sequence[LanguageConfig] | list[LanguageConfig]):
        ...
        language_patterns = [pattern for config in configs for pattern in config.filename_patterns]
        language_extensions = [extension for config in configs for extension in config.extensions]

        if len(language_names) != len(set(language_names)):
            duplicates = [name for name, count in Counter(language_names).items() if count > 1]
            raise ValueError(f"Duplicate language names: {duplicates}")

        if len(language_patterns) != len(set(language_patterns)):
            duplicates = [name for name, count in Counter(language_patterns).items() if count > 1]
            raise ValueError(f"Duplicate language patterns: {duplicates}")

        if len(language_extensions) != len(set(language_extensions)):
            duplicates = [name for name, count in Counter(language_extensions).items() if count > 1]
